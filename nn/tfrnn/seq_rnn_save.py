...
with tf.Session() as sess:
    ...
    for trnf in trnlist:
        ...
        batchesPerEpoch = nTrnSeq//batchsize + 1

        imgIdx_trn = [i for i in range(nTrnSeq)]
        nextra_trn = batchesPerEpoch*batchsize - nTrnSeq
        ...
        for e in range(nEpoch):
            ...
            for ib in range(batchesPerEpoch):
                ...
                middle1 = end
                middle2 = end
                if ib == batchesPerEpoch-1:
                    middle1 = nTrnSeq
                    middle2 = 0
                    end = nextra_trn
                yin = np.asarray([trn_seq[i_img+seq_len] \
                        for i_img in imgIdx_trn[start:middle1]+imgIdx_trn[middle2:end]])
            ...
            if e % epochEval == 0:
                ...
                for tb in range(ntestbatches):
                    ...
                    yin = np.asarray([test_seq[i_img+seq_len] \
                            for i_img in imgIdx_test[start:middle1]+imgIdx_test[middle2:end]])
